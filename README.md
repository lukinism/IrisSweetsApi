...
// С прокси
$api = new IrisSweets($botId, $irisToken, 'http://proxy.example.com:8080');
```
...
// Автоматически загружает настройки из .env (включая прокси, если указан)
$api = new IrisSweets();

// Или передайте параметры напрямую
$api = new IrisSweets('your-bot-id', 'your-iris-token');

// С прокси через конструктор
$api = new IrisSweets('your-bot-id', 'your-iris-token', 'http://proxy.example.com:8080');
```

### 🌐 Работа с прокси

Библиотека поддерживает работу через прокси-сервер. Прокси можно настроить несколькими способами:
...
```php
// HTTP прокси
$api = new IrisSweets($botId, $token, 'http://proxy.example.com:8080');

// HTTP прокси с авторизацией
$api = new IrisSweets($botId, $token, 'http://user:password@proxy.example.com:8080');

// SOCKS5 прокси
$api = new IrisSweets($botId, $token, 'socks5://127.0.0.1:1080');

// Простой формат (host:port) - будет использован HTTP
$api = new IrisSweets($botId, $token, 'proxy.example.com:8080');
```

#### Способ 3: Динамическое изменение прокси

```php
$api = new IrisSweets($botId, $token);

// Установить прокси
$api->setProxy('http://proxy.example.com:8080');

// Получить текущий прокси
$currentProxy = $api->getProxy();
...
- **HTTPS**: `https://host:port` или `https://user:pass@host:port`
- **SOCKS5**: `socks5://host:port` или `socks5://user:pass@host:port`
- **Простой формат**: `host:port` (будет использован HTTP)

**Примечание:** Если прокси не указан, библиотека работает напрямую без прокси.
...
- **Прокси настройки** можно хранить в `.env` файле или передавать программно

### Работа через прокси

Библиотека поддерживает работу через прокси-серверы для:
- Обхода ограничений доступа
- Повышения безопасности соединений
- Распределения нагрузки

**Рекомендации:**
- Используйте надежные прокси-серверы
- Не храните пароли прокси в открытом виде в коде
